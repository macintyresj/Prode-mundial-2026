...
// ── Subcomponente para cada partido eliminatorio (pronóstico) ──
function KnockoutCard({
  match, pred, res, onSave, isAdmin = false
}: {
  match: KnockoutMatch
  pred?: KnockoutPred
  res?: KnockoutResult
  onSave: (matchId: string, winnerId: string, penalties: boolean) => void
  isAdmin?: boolean
}) {
  ...
  const btnTeam = (selected: boolean): React.CSSProperties => ({
    flex: 1, padding: '10px 8px', borderRadius: 8,
    border: selected ? '2px solid var(--accent)' : '2px solid var(--border)',
    background: selected ? 'rgba(245,158,11,.12)' : 'var(--surface2)',
    color: selected ? 'var(--accent)' : 'var(--text)',
    fontFamily: 'Barlow,sans-serif', fontSize: 13, fontWeight: 600,
    cursor: 'pointer', textAlign: 'center', transition: 'all .15s'
  })
  const btnPen = (selected: boolean): React.CSSProperties => ({
    flex: 1, padding: '8px', borderRadius: 8,
    border: selected ? '2px solid var(--purple)' : '2px solid var(--border)',
    background: selected ? 'rgba(167,139,250,.12)' : 'var(--surface2)',
    color: selected ? 'var(--purple)' : 'var(--muted)',
    fontFamily: 'Barlow Condensed,sans-serif', fontSize: 12, fontWeight: 700,
    letterSpacing: 1, cursor: 'pointer', textAlign: 'center', transition: 'all .15s'
  })
  ...
  const statusBadge = () => {
    if (res) return <span style={{ fontSize: 10, fontFamily: 'Barlow Condensed,sans-serif', letterSpacing: 2, textTransform: 'uppercase', padding: '3px 7px', borderRadius: 4, background: 'rgba(167,139,250,.15)', color: 'var(--purple)' }}>Finalizado</span>
    if (!canPredict(match.fecha)) return <span style={{ fontSize: 10, fontFamily: 'Barlow Condensed,sans-serif', letterSpacing: 2, textTransform: 'uppercase', padding: '3px 7px', borderRadius: 4, background: 'rgba(100,116,139,.15)', color: 'var(--muted)' }}>Cerrado</span>
    const t = timeUntilLock(match.fecha)
          ...
          {can ? (
            <>
              ...
              <div style={{ display: 'flex', gap: 8, marginBottom: 12 }}>
                <button style={btnTeam(selWinner === match.teamA)} onClick={() => setSelWinner(match.teamA)}>{match.flagA} {match.teamA}</button>
                <button style={btnTeam(selWinner === match.teamB)} onClick={() => setSelWinner(match.teamB)}>{match.flagB} {match.teamB}</button>
              </div>
              ...
              <div style={{ display: 'flex', gap: 8, marginBottom: 12 }}>
                <button style={btnPen(selPenalties === true)} onClick={() => setSelPenalties(true)}>🥅 SÍ, penales</button>
                <button style={btnPen(selPenalties === false)} onClick={() => setSelPenalties(false)}>⚡ NO, tiempo reglamentario</button>
              </div>
              <button
                style={{ width: '100%', background: selWinner && selPenalties !== null ? 'var(--accent)' : 'var(--surface2)', border: 'none', borderRadius: 6, padding: '8px 0', color: selWinner && selPenalties !== null ? '#000' : 'var(--muted)', fontFamily: 'Barlow Condensed,sans-serif', fontSize: 13, fontWeight: 700, letterSpacing: 1, cursor: selWinner && selPenalties !== null ? 'pointer' : 'not-allowed' }}
                onClick={() => { if (selWinner && selPenalties !== null) onSave(match.id, selWinner, selPenalties) }}
            ...
            <>
              {pred && !res && (
                <div style={{ background: 'rgba(245,158,11,.08)', borderRadius: 8, padding: '10px 14px', fontSize: 13, color: 'var(--accent)', textAlign: 'center' }}>
                  🔒 Tu pronóstico: <strong>{pred.winnerId}</strong> · {pred.penalties ? '🥅 Con penales' : '⚡ Sin penales'}
                </div>
              )}
              {res && (
                <div style={{ marginTop: 4 }}>
                  <div style={{ fontSize: 13, color: 'var(--green)', fontWeight: 600, marginBottom: 4 }}>
                    ✅ Ganó: {res.winnerId} · {res.penalties ? '🥅 Fue a penales' : '⚡ Sin penales'}
                  </div>
                  {pred ? (
                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', paddingTop: 8, borderTop: '1px solid var(--border)' }}>
                      <div style={{ fontSize: 12, color: 'var(--muted)' }}>{pred.winnerId} · {pred.penalties ? 'Con penales' : 'Sin penales'}</div>
                      <div style={{ fontFamily: 'Bebas Neue,cursive', fontSize: 20, color: 'var(--gold)' }}>+{calcPointsKnockout(pred.winnerId, pred.penalties, res.winnerId, res.penalties)}pts</div>
                    </div>
                  ) : <div style={{ fontSize: 12, color: 'var(--muted)' }}>Sin pronóstico</div>}
                </div>
              )}
              {!pred && !res && hasTeams && <div style={{ fontSize: 12, color: 'var(--muted)', textAlign: 'center', padding: '8px 0' }}>⏰ Pronóstico cerrado</div>}
            </>
          )}
        </>
      )}
...
// ── Subcomponente admin para cada partido eliminatorio ──
function AdminKnockoutCard({
  match, res, onSaveTeams, onSaveResult
}: {
  match: KnockoutMatch
  res?: KnockoutResult
  onSaveTeams: (matchId: string, teamA: string, teamB: string) => void
  onSaveResult: (matchId: string, winnerId: string, penalties: boolean) => void
}) {
  ...
  const matchCard: React.CSSProperties = { background: 'var(--surface)', border: `1px solid ${hasTeams ? 'var(--border)' : 'rgba(245,158,11,.2)'}`, borderRadius: 10, padding: '12px 16px', marginBottom: 8 }
  ...
  const btnPen = (selected: boolean): React.CSSProperties => ({ flex: 1, padding: '8px', borderRadius: 8, border: selected ? '2px solid var(--purple)' : '2px solid var(--border)', background: selected ? 'rgba(167,139,250,.12)' : 'var(--surface2)', color: selected ? 'var(--purple)' : 'var(--muted)', fontFamily: 'Barlow Condensed,sans-serif', fontSize: 12, fontWeight: 700, letterSpacing: 1, cursor: 'pointer', textAlign: 'center', transition: 'all .15s' })
  const inp: React.CSSProperties = { flex: 1, background: 'var(--surface2)', border: '1px solid var(--border)', borderRadius: 6, padding: '8px 10px', color: 'var(--text)', fontFamily: 'Barlow,sans-serif', fontSize: 13, outline: 'none' }
  ...
  return (
    <div style={matchCard}>
      ...
      <div style={{ display: 'flex', gap: 8, marginBottom: 12, alignItems: 'center' }}>
        <input style={inp} value={teamA} onChange={e => setTeamA(e.target.value)} placeholder="Ej: Argentina 🇦🇷" />
        <input style={inp} value={teamB} onChange={e => setTeamB(e.target.value)} placeholder="Ej: Francia 🇫🇷" />
        <button style={{ background: 'var(--surface2)', border: '1px solid var(--border)', borderRadius: 6, padding: '8px 12px', color: 'var(--text)', fontFamily: 'Barlow Condensed,sans-serif', fontSize: 12, cursor: 'pointer', whiteSpace: 'nowrap' }}
          onClick={() => onSaveTeams(match.id, teamA, teamB)}>💾 Guardar</button>
      </div>
      {hasTeams && (
        <>
          ...
          <div style={{ display: 'flex', gap: 8, marginBottom: 8 }}>
            <button style={btnTeam(adminWinner === match.teamA)} onClick={() => setAdminWinner(match.teamA)}>{match.flagA || ''} {match.teamA}</button>
            <button style={btnTeam(adminWinner === match.teamB)} onClick={() => setAdminWinner(match.teamB)}>{match.flagB || ''} {match.teamB}</button>
          </div>
          <div style={{ display: 'flex', gap: 8, marginBottom: 8 }}>
            <button style={btnPen(adminPenalties === true)} onClick={() => setAdminPenalties(true)}>🥅 SÍ, fue a penales</button>
            <button style={btnPen(adminPenalties === false)} onClick={() => setAdminPenalties(false)}>⚡ NO, tiempo reglamentario</button>
          </div>
          <button
            style={{ width: '100%', background: adminWinner && adminPenalties !== null ? 'var(--green)' : 'var(--surface2)', border: 'none', borderRadius: 6, padding: '8px 0', color: adminWinner && adminPenalties !== null ? '#000' : 'var(--muted)', fontFamily: 'Barlow Condensed,sans-serif', fontSize: 13, fontWeight: 700, letterSpacing: 1, cursor: adminWinner && adminPenalties !== null ? 'pointer' : 'not-allowed' }}
            onClick={() => { if (adminWinner && adminPenalties !== null) onSaveResult(match.id, adminWinner, adminPenalties) }}
...
export default function GroupPage() {
  ...
  const loadData = useCallback(async (uid: string) => {
    ...
    const savedMatches: Record<string, Partial<KnockoutMatch>> = {}
    koMatchSnap.docs.forEach(d => { savedMatches[d.id] = d.data() })
  ...
  async function saveKnockoutTeams(matchId: string, teamA: string, teamB: string) {
  ...
  const tabStyle = (t: Tab): React.CSSProperties => ({ flexShrink: 0, padding: '12px 16px', fontFamily: 'Barlow Condensed,sans-serif', fontSize: 11, fontWeight: 600, letterSpacing: 2, textTransform: 'uppercase', cursor: 'pointer', borderBottom: tab === t ? '2px solid var(--accent)' : '2px solid transparent', color: tab === t ? 'var(--accent)' : 'var(--muted)', whiteSpace: 'nowrap' })
  ...
  const statusBadgeGroup = (fecha: string, hasResult: boolean) => {
      ...
      <div style={{ padding: 18, maxWidth: 860, margin: '0 auto', width: '100%' }}>
        ...
        {tab === 'posiciones' && (
          <>
            ...
            <div style={{ marginTop: 16, background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: 10, padding: '12px 16px', fontSize: 13, lineHeight: 2 }}>
              <div style={{ fontSize: 11, letterSpacing: 2, textTransform: 'uppercase', color: 'var(--muted)', marginBottom: 4, fontFamily: 'Barlow Condensed,sans-serif' }}>Sistema de puntos</div>
                  ...
                  return (
                    <div key={match.id} style={matchCard}>
                      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 10 }}>
                        ...
                        {statusBadgeGroup(match.fecha, !!res)}
                      </div>
                ...
                <div key={fase}>
                  ...
                  {matches.map(match => (
                    <KnockoutCard
                      key={match.id}
                      match={match}
                      pred={getKoPred(match.id)}
                      res={getKoResult(match.id)}
                      onSave={saveKnockoutPred}
                    />
                  ))}
                </div>
              )
                ...
                {FIXTURE.filter(m => m.grupo === g).map(match => {
                  ...
                  return (
                    <div key={match.id} style={matchCard}>
                      ...
                      <div style={{ display: 'grid', gridTemplateColumns: '1fr auto 1fr', alignItems: 'center', gap: 8 }}>
                        <div style={{ display: 'flex', alignItems: 'center', gap: 6, fontWeight: 600, fontSize: 13 }}><span style={{ fontSize: 20 }}>{match.flagLocal}</span>{match.local}</div>
                        <div>{res ? <span style={{ fontFamily: 'Bebas Neue,cursive', fontSize: 26, color: 'var(--gold)' }}>{res.goalsLocal} : {res.goalsVisitor}</span> : <span style={{ fontSize: 15, color: 'var(--muted)', padding: '0 8px' }}>vs</span>}</div>
                        <div style={{ display: 'flex', alignItems: 'center', flexDirection: 'row-reverse', gap: 6, fontWeight: 600, fontSize: 13 }}><span style={{ fontSize: 20 }}>{match.flagVisitante}</span>{match.visitante}</div>
                      </div>
          ...
          <>
            ...
            <div style={{ background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: 10, padding: '14px 18px', marginBottom: 16, display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
              ...
              <button onClick={() => navigator.clipboard.writeText(group?.code || '').then(() => showToast('Código copiado!', 'success'))}
                style={{ background: 'var(--surface2)', border: '1px solid var(--accent)', borderRadius: 6, padding: '8px 14px', color: 'var(--accent)', fontFamily: 'Barlow Condensed,sans-serif', fontSize: 13, cursor: 'pointer' }}>📋 Copiar</button>
            </div>
            {lb.map((u, i) => (
        ...
        {tab === 'admin' && isAdmin && (
          <>
            ...
            <div style={{ background: 'rgba(167,139,250,.08)', border: '1px solid rgba(167,139,250,.2)', borderRadius: 8, padding: '10px 14px', fontSize: 13, color: 'var(--purple)', marginBottom: 16 }}>
              👑 Los pronósticos se cierran 2 horas antes de cada partido.
            </div>
            {grupos.map(g => (
              <div key={g}>
                <div style={grupTitle}>Grupo {g}<div style={{ flex: 1, height: 1, background: 'var(--border)' }} /></div>
            ...
            <div style={{ background: 'rgba(245,158,11,.08)', border: '1px solid rgba(245,158,11,.2)', borderRadius: 8, padding: '10px 14px', fontSize: 13, color: 'var(--accent)', marginBottom: 16 }}>
              ⚔️ Primero cargá los equipos de cada cruce. Luego confirmá el resultado cuando se juegue.
            </div>
            {fases.map(fase => {
              const matches = knockoutMatches.filter(m => m.fase === fase)
              return (
